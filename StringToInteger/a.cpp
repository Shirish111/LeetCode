...
// Tags: string_manipulation, number
    ...
    for (int i = 0; i < l; i++) {
      switch (state) {
        ...
        case 2:
          if (s[i] >= '0' && s[i] <= '9') {
            if (pos &&
                (num > INT_MAX / 10 || (num == INT_MAX / 10 && s[i] >= '8'))) {
              return INT_MAX;
            }
            if (!pos &&
                (num < INT_MIN / 10 || (num == INT_MIN / 10 && s[i] > '8'))) {
              return INT_MIN;
            }
            if (pos) {
              num = num * 10 + s[i] - '0';
...
void solve() {
  string test_cases[] = {"+- 123",        "+-123",       "1abcd",
                         "  +2147483647", "-2147483649", "2147483648"};
  int num_cases = sizeof(test_cases) / sizeof(test_cases[0]);
  Solution s;
  F(i, 0, num_cases) { cout << s.myAtoi(test_cases[i]) << "\n"; }
}
int main() {
  cin.tie(nullptr);
